...
const OrderPageTable = React.memo(({ infoOrder, objectProducts, increment }: OrderPageTableProps) => {
  ...
  <>
   {infoOrder.map(position =>
    <ListGroup key={'position-group' + position.productId} horizontal={'xxl'} className="my-2">
     ...
     {!isDone[position.productId] && <ListGroup.Item className='flex-grow-1 text-center' key={'position-done-btn' + position.productId}>
      <Button onClick={() => {
